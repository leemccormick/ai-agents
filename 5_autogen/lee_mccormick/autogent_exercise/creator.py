...
class Creator(RoutedAgent):
    ...
    async def handle_my_message_type(self, message: messages.Message, ctx: MessageContext) -> messages.Message:
        filename = message.content  # e.g., "agentbob.py"
        agent_name = filename.split(".")[0]  # e.g., "agentbob"

        # 1. Create agents/ folder if it doesn't exist
        agent_dir = "agents"
        os.makedirs(agent_dir, exist_ok=True)

        # 2. Compose full path to the file
        full_path = os.path.join(agent_dir, filename)

        # 3. Run the agent's internal generation logic
        text_message = TextMessage(content=self.get_user_prompt(), source="user")
        response = await self._delegate.on_messages([text_message], ctx.cancellation_token)

        # 4. Save generated agent code to agents/filename.py
        ...
        # 5. Dynamically import from the agents/ folder
        module = importlib.import_module(f"agents.{agent_name}")

        # 6. Register and activate the agent
